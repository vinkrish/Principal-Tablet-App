...
public class ExmAvgDao {
    ...
    public static void insertExmAvg(long examId, int subjectId, SQLiteDatabase sqliteDatabase) {
        String sql = "SELECT A.ExamId, A.SubjectId, B.ClassId, B.SectionId, (AVG(Mark)/C.MaximumMark)*360 as Average FROM marks A, students B, subjectexams C WHERE A.StudentId = B.StudentId" +
                " and A.Mark!='-1' and A.ExamId=" + examId + " and C.ExamId=A.ExamId and A.SubjectId=" + subjectId + " and C.SubjectId=A.SubjectId GROUP BY A.ExamId, A.SubjectId, B.SectionId";
        Cursor c = sqliteDatabase.rawQuery(sql, null);
        c.moveToFirst();
        while (!c.isAfterLast()) {
            try {
    ...
    public static int seSecAvg2(int sectionId, long examId, SQLiteDatabase sqliteDatabase) {
        double avg = 0;
    ...
    public static int selectSeAvg2d(int sectionId, int subjectId, long examId, SQLiteDatabase sqliteDatabase) {
        Cursor c = sqliteDatabase.rawQuery("select Avg(ExamAvg) as avg from exmavg where SectionId=" + sectionId + " and SubjectId=" + subjectId + " and ExamId=" + examId, null);
        c.moveToFirst();
        int avg = 0;
        while (!c.isAfterLast()) {
    ...
    public static int getSeExamAvgd(long examId, int sectionId, SQLiteDatabase sqliteDatabase) {
        double avg = 0;
